...
static void SetuptocoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " tocoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The tocoin-tx tool is used for creating and modifying tocoin transactions.\n\n"
                "tocoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded tocoin transaction, or with \"-create [commands]\" to create a hex-encoded tocoin transaction.\n"
                "\n"
                "Usage: tocoin-tx [options] <hex-tx> [commands]\n"
                "or:    tocoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded tocoin transaction
