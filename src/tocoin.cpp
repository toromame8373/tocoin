// Copyright (c) 2025-present The tocoin Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries tocoin-node, tocoin-gui.
  -M, --monolithic       Run monolithic binaries tocoind, tocoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'tocoin-qt [ARGS]' or 'tocoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'tocoind [ARGS]' or 'tocoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'tocoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'tocoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'tocoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_tocoin [ARGS]'.
  chainstate [ARGS] Run tocoin kernel chainstate util, equivalent to running 'tocoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_tocoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_tocoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "tocoin-gui" : "tocoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "tocoin-node" : "tocoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("tocoin-cli");
            // Since "tocoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("tocoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("tocoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_tocoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("tocoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_tocoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_tocoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("tocoin-util");
        } else {
...
//! Execute the specified tocoind, tocoin-qt or other command line in `args`
...
//!             be a relative path to a tocoind, tocoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/tocoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/tocoin/tocoin/pull/31375#discussion_r1861814807)
